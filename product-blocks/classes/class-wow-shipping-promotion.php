 ...
 *
 * @package WOPB\WowShippingPromotion
 */
...
class WowShippingPromotion {
	...
	public function add_submenu() {
		...
		$url = admin_url( 'admin.php?page=wopb-settings#our_product/wow_shipping' ); // CHANGE THIS.
		...
		<style>
			...
			#wtrs-submenu-link,
			#productx-submenu-our_product\/wow_shipping {
	...
	public function ajax_install_promotion_plugin() {
		...
		if ( ! $plugin_exists && ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'You are not allowed to install plugins.', 'product-blocks' ),
				)
			);
		}
		...
		if ( $plugin_exists && ! current_user_can( 'activate_plugins' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'You are not allowed to activate plugins.', 'product-blocks' ),
				)
			);
		}
		...
		if ( false === $result ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Failed to install WowShipping.', 'product-blocks' ),
				)
			);
		}
		...
		wp_send_json_success(
			array(
				...
				'message'       => esc_html__( 'WowShipping installed successfully.', 'product-blocks' ),
			)
		);
	}
	...
	private function render_promotion_notice( $id, $type, $message, $style = '', $inline = true, $button_labels = array() ) {
		...
		>
			...
			<button type="button" class="notice-dismiss">
				<span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'product-blocks' ); ?></span>
			</button>
		</div>
		...
		<script>
			jQuery( function( $ ) {
				$( document ).on( 'click', '<?php echo esc_js( $id ); ?> .wtrs-promotion-install-link', function( event ) {
					...
					var defaultErrorMessage = <?php echo wp_json_encode( esc_html__( 'Failed to install WowShipping.', 'product-blocks' ) ); ?>;
