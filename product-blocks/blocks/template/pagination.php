...
if ( is_product_taxonomy() && ! isset( $attr['product_filters'] ) ) {
	$filter_attributes['productTaxonomy'] = array(
...
$wrapper_main_content     .= '<div data-archive-builder="' . ( is_archive() || is_search() || is_product_taxonomy() || is_product_tag() && ! is_shop() ) . '" data-search-block="' . is_search() . '" class="wopb-pagination-wrap' . ( $attr['paginationAjax'] ? ' wopb-pagination-ajax-action' : '' ) . '" data-paged="1" data-blockid="' . esc_attr( $attr['blockId'] ) . '" data-postid="' . esc_attr( $page_post_id ) . '" data-pages="' . esc_attr( $pageNum ) . '" data-blockname="product-blocks_' . esc_attr( $block_name ) . '" ' . wopb_function()->get_builder_attr() . $data_filter_attributes . '>';
