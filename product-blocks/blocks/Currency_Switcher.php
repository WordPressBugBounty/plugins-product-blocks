...
class Currency_Switcher {
	...
	public function register() {
		register_block_type(
			'product-blocks/currency-switcher',
			array(
	...
	public function content( $attr, $noAjax = false ) {
		...
		if ( wopb_function()->get_setting( 'wopb_currency_switcher' ) === 'true' && wopb_function()->get_setting( 'is_lc_active' ) ) {
			...
			foreach ( wopb_function()->get_setting( 'wopb_currencies' ) as $key => $currency ) {
				...
				} elseif ( $attr['currencySymbolPosition'] == 'rightDollar' ) {
			...
			if ( $attr['countryNameShow'] ) {
				$wraper_before .= '<div class="wopb-selected-currency" value="">' . ( $attr['showFlag'] ? '<img src="' . esc_url( 'https://raw.githubusercontent.com/wpxpo/wpxpo_profile/main/country_flags/' . strtolower( $wopb_current_currency ) . '.png' ) . '" alt="flag"> ' : '' ) . $added_currency[ $wopb_current_currency ] . '</div>'; // phpcs:ignore PluginCheck.CodeAnalysis.Offloading.OffloadedContent
			} else {
				$Code = '/\b[A-Z]{3}\b/';
				if ( preg_match( $Code, $added_currency[ $wopb_current_currency ], $currencyMatches ) ) {
					$wraper_before .= '<div class="wopb-selected-currency" value="">' . ( $attr['showFlag'] ? '<img src="' . esc_url( 'https://raw.githubusercontent.com/wpxpo/wpxpo_profile/main/country_flags/' . strtolower( $wopb_current_currency ) . '.png' ) . '" alt="flag"> ' : '' ) . $currencyMatches[0] . '</div>'; // phpcs:ignore PluginCheck.CodeAnalysis.Offloading.OffloadedContent
				}
			}
			...
			if ( isset( $added_currency ) && count( $added_currency ) > 1 ) {
				...
				foreach ( $added_currency as $key => $label ) {
					if ( $attr['countryNameShow'] ) {
						$wraper_before .= '<li class="' . ( $wopb_current_currency == $key ? 'hide-currency' : '' ) . '" value="' . esc_attr( $key ) . '">' . ( $attr['showFlag'] ? '<img src="' . esc_url( 'https://raw.githubusercontent.com/wpxpo/wpxpo_profile/main/country_flags/' . strtolower( $key ) . '.png' ) . '" alt="flag"> ' : '' ) . wp_strip_all_tags( esc_html( $label ) ) . '</li>'; // phpcs:ignore PluginCheck.CodeAnalysis.Offloading.OffloadedContent
					} else {
						$currency = '/\( ([^)]+) \)/';
						if ( preg_match( $currency, $label, $currencyMatches ) ) {
							$wraper_before .= '<li class="' . ( $wopb_current_currency == $key ? 'hide-currency' : '' ) . '" value="' . esc_attr( $key ) . '">' . ( $attr['showFlag'] ? '<img  src="' . esc_url( 'https://raw.githubusercontent.com/wpxpo/wpxpo_profile/main/country_flags/' . strtolower( $key ) . '.png' ) . '" alt="flag"> ' : '' ) . wp_strip_all_tags( $currencyMatches[0] . ' ' . $key ) . '</li>'; // phpcs:ignore PluginCheck.CodeAnalysis.Offloading.OffloadedContent
						}
					}
				}
							$wraper_before .= '</ul>';
